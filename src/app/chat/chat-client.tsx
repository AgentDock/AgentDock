...
function ChatPageContent() {
  ...
  const handleReset = useCallback(async () => {
    try {
      ...
      logger.error(LogCategory.API, 'ChatPage', 'Failed to reset chat', {
  ...
  const checkTokenWarning = useCallback(
    ...
    [hasShown50kWarning, hasShown100kWarning],
...
export default function ChatClientPage() {
  ...
  // Get agent name for the skeleton if possible
  const agentName =
    agentId && templates[agentId as TemplateId] ? templates[agentId as TemplateId].name : undefined;
  ...
  const fallback = isFirstLoad ? <ChatSkeleton agentName={agentName} /> : <ChatSkeleton />; // Provide a default skeleton if not first load but still suspending
