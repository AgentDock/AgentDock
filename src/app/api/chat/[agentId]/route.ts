 ...
 */
async function handleDirectToolExecution(request: NextRequest, body: any, agentId: string) {
  try {
    ...
    if (!template) {
      return NextResponse.json({ error: `Template not found: ${agentId}` }, { status: 404 });
    }
    ...
    if (sessionIdHeader && template && 'orchestration' in template) {
      try {
        ...
        logger.debug(LogCategory.API, 'ChatRoute', 'Tracked tool usage for orchestration', {
          agentId,
          toolName,
          sessionId: sessionIdHeader.substring(0, 8) + '...',
        });
      } catch (error) {
        logger.warn(LogCategory.API, 'ChatRoute', 'Failed to track tool usage', {
...
export async function POST(
  ...
  try {
    ...
    const template = templates[agentId as TemplateId];
    if (!template) {
      throw new APIError('Template not found', ErrorCode.CONFIG_NOT_FOUND, request.url, 'POST', {
        agentId,
      });
    }
    ...
    const finalSessionId =
      clientSessionId || `session-${agentId}-${Date.now()}-${crypto.randomUUID()}`;

    // Log request details
    const sessionIdDisplay = finalSessionId ? `${finalSessionId.substring(0, 12)}...` : 'none';
    await logger.debug(LogCategory.API, 'ChatRoute', 'Processing chat request with final details', {
      agentId,
      messageCount: messages.length,
      model: llmConfig.model,
    ...
    if (template && 'orchestration' in template && template.orchestration) {
      ...
      if (finalSessionId) {
        ...
        orchestrationState = await import('@/lib/orchestration-adapter').then((module) =>
          module.getOrchestrationState(finalSessionId, mutableConfig),
        );

        logger.debug(LogCategory.API, 'ChatRoute', 'Retrieved orchestration state', {
          agentId,
