...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Debug logging
    logger.debug(LogCategory.API, 'ImageStore', `GET request for image ID: ${id}`, {
      url: request.url,
    });

    if (!id) {
      logger.warn(LogCategory.API, 'ImageStore', 'Missing image ID in request');
    ...
    if (!image) {
      logger.warn(LogCategory.API, 'ImageStore', `Image not found with ID: ${id}`);
      return new Response('Image not found', { status: 404 });
    }

    // Log successful image retrieval
    logger.info(LogCategory.API, 'ImageStore', `Successfully retrieved image with ID: ${id}`, {
    ...
    logger.error(LogCategory.API, 'ImageStore', 'Error retrieving image', {
