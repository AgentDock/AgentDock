...
async function validateVercelConfig(): Promise<boolean> {
  ...
  const requiredFields = ['buildCommand', 'installCommand', 'framework', 'outputDirectory'];
...
async function validateRouteManifest(): Promise<boolean> {
  ...
  try {
    ...
    const hasAppApiRoutes = Object.keys(manifest).some((route) => route.startsWith('/api/'));

    // Check for our API routes in pages directory
    const hasPagesApiRoutes = Object.keys(pagesManifest).some((route) => route.startsWith('/api/'));
    ...
    const dynamicRoutes = execSync('find src/app/api -name "route.ts" -type f', {
      encoding: 'utf8',
    });
