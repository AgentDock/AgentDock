...
import { SemanticMemory, MemoryType } from '../../../types';
...
describe('SemanticMemory - Actual Implementation', () => {
  ...
  describe('Configuration Requirements (NO DEFAULTS)', () => {
    test('requires storage with memory operations', () => {
      ...
      expect(() => new SemanticMemory(storageWithoutMemory, testConfig.memory.semantic))
    ...
    test('uses vectorSearchEnabled from configuration', async () => {
      ...
      const results = await semanticMemory.search(userId, agentId, 'test query');
  ...
  describe('User Isolation - CRITICAL SECURITY', () => {
    ...
    test('enforces user isolation on search operations', async () => {
      ...
      await semanticMemory.store(testConfig.users.alice, testConfig.agents.shared, 'Alice fact about AI', { keywords: ['AI', 'fact'] });
      await semanticMemory.store(testConfig.users.bob, testConfig.agents.shared, 'Bob fact about AI', { keywords: ['AI', 'fact'] });
      
      // Search should only return user's own memories
      const aliceResults = await semanticMemory.search(testConfig.users.alice, testConfig.agents.shared, 'AI');
      const bobResults = await semanticMemory.search(testConfig.users.bob, testConfig.agents.shared, 'AI');
    ...
    test('requires userId for ALL operations', async () => {
      ...
      await expect(semanticMemory.store('', agentId, 'content'))
        .rejects.toThrow('userId is required for semantic memory operations');
      
      await expect(semanticMemory.search('', agentId, 'query'))
        .rejects.toThrow('userId is required for semantic memory operations');
      
      await expect(semanticMemory.consolidate('', agentId))
        .rejects.toThrow('userId is required for semantic memory operations');
      
      await expect(semanticMemory.getStats(''))
        .rejects.toThrow('userId is required for semantic memory operations');
      
      await expect(semanticMemory.getById('', 'memoryId'))
        .rejects.toThrow('userId is required for semantic memory operations');
    });
  });
  ...
  describe('Storage Delegation - Core Architecture', () => {
    ...
    test('delegates getById operations to storage layer', async () => {
      ...
      const userId = testConfig.users.alice;
      const memoryId = await semanticMemory.store(userId, testConfig.agents.shared, 'Test semantic content');
  ...
  describe('Facts and Relations - Advanced Features', () => {
    ...
    test('supports confidence scoring for probabilistic truth values', async () => {
      ...
      const memoryId = await semanticMemory.store(userId, agentId, 'Uncertain fact', {
        confidence: 0.6
      });
      ...
      const memoryId2 = await semanticMemory.store(userId, agentId, 'Default confidence fact');
      
      const stored2 = await storage.memory.getById!(userId, memoryId2);
      expect(stored2?.metadata?.confidence).toBe(testConfig.memory.semantic.confidenceThreshold); // Config value (0.5)
    });
    ...
    test('supports source attribution for knowledge provenance', async () => {
      ...
      const memoryId = await semanticMemory.store(userId, agentId, 'Fact from Wikipedia', {
        source: 'https://en.wikipedia.org/wiki/Example'
      });
      
      const stored = await storage.memory.getById!(userId, memoryId);
      expect(stored?.metadata?.source).toBe('https://en.wikipedia.org/wiki/Example');
    });
  });
  ...
  describe('Keywords and Category Management', () => {
    test('stores and manages keywords for semantic organization', async () => {
      ...
      const memoryId = await semanticMemory.store(userId, agentId, 'ML knowledge', {
        keywords
      });
    ...
    test('handles empty keywords gracefully', async () => {
      ...
      const memoryId = await semanticMemory.store(userId, agentId, 'Knowledge without keywords');
  ...
  describe('Memory Data Validation and Mapping', () => {
    test('properly maps storage data to SemanticMemoryData on getById', async () => {
      ...
      const memoryId = await semanticMemory.store(userId, agentId, 'Semantic test content', {
    ...
    test('returns null for non-existent memory', async () => {
      const result = await semanticMemory.getById(testConfig.users.alice, 'non-existent-id');
  ...
  describe('Statistics and Monitoring', () => {
    test('returns semantic memory stats through storage', async () => {
      ...
      await semanticMemory.store(userId, agentId, 'Fact 1', { keywords: ['fact'] });
      await semanticMemory.store(userId, agentId, 'Fact 2', { keywords: ['fact'] });
  ...
  describe('BaseMemoryType Integration', () => {
    test('inherits automatic connection discovery from BaseMemoryType', async () => {
      ...
      const memoryId = await semanticMemory.store(userId, agentId, 'Semantic content for connection test');
  ...
  describe('Error Handling', () => {
    test('handles storage errors gracefully', async () => {
      ...
      await expect(semanticMemory.store(testConfig.users.alice, testConfig.agents.shared, 'Test'))
    ...
    test('handles storage without getById method', async () => {
      ...
      const limitedSemanticMemory = new SemanticMemory(limitedStorage, testConfig.memory.semantic);
      
      const result = await limitedSemanticMemory.getById(testConfig.users.alice, 'any-id');
      expect(result).toBeNull();
    });
  });
  ...
  describe('Configuration-Driven Behavior', () => {
    test('uses confidenceThreshold from configuration when no custom confidence provided', async () => {
      ...
      const memoryId = await semanticMemory.store(userId, agentId, 'Test knowledge');
      const stored = await storage.memory.getById!(userId, memoryId);
      
      // Should use testConfig.memory.semantic.confidenceThreshold (0.5), not hardcoded 1.0
      expect(stored?.metadata?.confidence).toBe(testConfig.memory.semantic.confidenceThreshold);
    ...
    test('respects encryptSensitive configuration', () => {
      ...
      expect(typeof testConfig.memory.semantic.encryptSensitive).toBe('boolean');
    });
    ...
    test('respects vectorSearchEnabled configuration', () => {
      ...
      expect(typeof testConfig.memory.semantic.vectorSearchEnabled).toBe('boolean');
    });
    ...
    test('respects autoExtractFacts configuration', () => {
      ...
      expect(typeof testConfig.memory.semantic.autoExtractFacts).toBe('boolean');
    });
  });
}); 