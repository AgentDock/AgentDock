...
export class OrchestrationManager {
  ...
  public async ensureStateExists(sessionId: SessionId): Promise<OrchestrationState | null> {
    logger.debug(
      LogCategory.ORCHESTRATION,
      'ensureStateExists',
      'Ensuring state exists for session',
      { sessionId },
    );
    try {
      ...
      logger.debug(LogCategory.ORCHESTRATION, 'ensureStateExists', 'State ensured/retrieved', {
        sessionId,
        stateExists: !!state,
      });
      return state;
  ...
  public async getActiveStep(
    ...
    messages: LLMMessage[],
    sessionId: SessionId,
  ): Promise<OrchestrationStep | undefined> {
    // Changed to Promise
    // If no orchestration, return undefined
    if (!orchestration?.steps?.length) return undefined;

    logger.debug(LogCategory.ORCHESTRATION, 'getActiveStep', 'Starting step evaluation', {
      sessionId,
    }); // Log Start
    ...
    if (!state) {
      logger.warn(LogCategory.ORCHESTRATION, 'getActiveStep', 'Failed to get or create state', {
        sessionId,
      });
    ...
    for (const step of orchestration.steps) {
      if (step.conditions?.length && !step.isDefault) {
        logger.debug(LogCategory.ORCHESTRATION, 'getActiveStep', 'Checking conditions for step', {
          sessionId,
          stepName: step.name,
        }); // Log checking step
        ...
        for (const condition of step.conditions) {
          ...
          logger.debug(LogCategory.ORCHESTRATION, 'getActiveStep', 'Condition check result', {
            sessionId,
            stepName: step.name,
        ...
        if (allConditionsMet) {
          logger.debug(LogCategory.ORCHESTRATION, 'getActiveStep', 'All conditions met for step', {
            sessionId,
            stepName: step.name,
          }); // Log conditions met
          if (state.activeStep !== step.name) {
            logger.info(LogCategory.ORCHESTRATION, 'getActiveStep', 'Transitioning active step', {
              sessionId,
            ...
            logger.debug(
              LogCategory.ORCHESTRATION,
              'getActiveStep',
              'Conditions met, but step is already active',
              { sessionId, stepName: step.name },
            ); // Log already active
          }
          return step;
        }
    ...
    if (state.activeStep) {
      ...
      if (currentStepInConfig) {
        logger.debug(
          LogCategory.ORCHESTRATION,
          'getActiveStep',
          'No transition conditions met, returning current step from state',
          { sessionId, stepName: state.activeStep },
        ); // Log returning current
        return currentStepInConfig;
      }
      logger.warn(
        LogCategory.ORCHESTRATION,
        'getActiveStep',
        'Active step in state not found in config, falling back to default',
        { sessionId, invalidStep: state.activeStep },
      );
    }
    ...
    if (defaultStep) {
      logger.debug(LogCategory.ORCHESTRATION, 'getActiveStep', 'Falling back to default step', {
        sessionId,
        defaultStepName: defaultStep.name,
      }); // Log falling back to default
      if (state.activeStep !== defaultStep.name) {
        // Also reset sequence index when falling back to default
        await this.stateManager.updateState(sessionId, {
          activeStep: defaultStep.name,
          sequenceIndex: 0,
        });
      }
      return defaultStep;
    ...
    logger.warn(LogCategory.ORCHESTRATION, 'getActiveStep', 'No active or default step found', {
      sessionId,
    }); // Log no step found
    return undefined;
  }
      ...
      case 'sequence_match': {
        ...
        if (!sequence || sequence.length === 0) {
          logger.warn(
            LogCategory.ORCHESTRATION,
            'checkCondition',
            'sequence_match condition used on step with no sequence',
            { stepName: step.name },
          );
  ...
  public async updateState(
    ...
    logger.debug(LogCategory.ORCHESTRATION, 'updateState', 'Updating state', {
      sessionId,
      keysToUpdate: Object.keys(partialState),
    });
    try {
      ...
      if (!updatedState) {
        ...
        logger.error(
          LogCategory.ORCHESTRATION,
          'updateState',
          'State update failed (stateManager.updateState returned null)',
          { sessionId },
        );
      }
      return updatedState;
  ...
  public async resetState(sessionId: SessionId): Promise<void> {
    logger.debug(LogCategory.ORCHESTRATION, 'resetState', 'Resetting state for session', {
      sessionId,
    });
    try {
   ...
   */
  public async removeSession(sessionId: SessionId): Promise<void> {
    // Changed to async Promise<void>
