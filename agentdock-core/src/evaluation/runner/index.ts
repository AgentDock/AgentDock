...
import {
  ...
  ToolUsageEvaluator,
  type ToolUsageEvaluatorConfig, // Added ToolUsage imports
  LexicalSimilarityEvaluator,
  type LexicalSimilarityEvaluatorConfig, // Added LexicalSimilarity imports
  KeywordCoverageEvaluator,
  type KeywordCoverageEvaluatorConfig, // Added
  SentimentEvaluator,
  type SentimentEvaluatorConfig, // Added
  ToxicityEvaluator,
  ...
  for (const [index, config] of evaluatorConfigs.entries()) {
    ...
    switch (config.type) {
      ...
      case 'KeywordCoverage': // Added
        ...
        if (
          (config.config.keywordsSourceField === 'config' || !config.config.keywordsSourceField) &&
          (!config.config.expectedKeywords || config.config.expectedKeywords.length === 0)
        ) {
          throw new Error(
