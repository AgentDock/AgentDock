...
import { IExtractor, ExtractionContext, ExtractorConfig } from '../types';
import { CostTracker } from '../tracking/CostTracker';
import { generateId } from '../../../storage/utils';
import { createLLM, CoreLLM } from '../../../llm';
...
export abstract class LLMExtractor implements IExtractor {
  ...
  constructor(config: ExtractorConfig, costTracker: CostTracker) {
    ...
    if (!config.provider || !config.model || !config.apiKey) {
      throw new Error('LLMExtractor requires provider, model, and apiKey configuration');
    }
    this.llm = createLLM({
  ...
  async extract(message: MemoryMessage, context: ExtractionContext): Promise<Memory[]> {
    try {
      ...
      logger.error(LogCategory.STORAGE, 'LLMExtractor.extract', 'Extraction failed', { error });
      return [];
    }
  }
   ...
   * @protected
   */
  protected buildExtractionPrompt(message: MemoryMessage, context: ExtractionContext): string {
    const rulesExamples = context.userRules
      .slice(0, 5) // Include up to 5 example rules
      .map(rule => `- Pattern: "${rule.pattern}" extracts ${rule.type} memories with importance ${rule.importance}`)
      .join('\n');
...
Empty array if nothing important: []`;
  ...
  protected async parseMemoriesFromResponse(
    ...
    try {
      ...
      logger.warn(LogCategory.STORAGE, 'LLMExtractor.parseMemories', 'Failed to parse response');
      return [];
    }
  }
