...
const stockPriceSchema = z.object({
  ...
  apiKey: z
    .string()
    .optional()
    .describe('Optional AlphaVantage API key (will use environment variable if not provided)'),
});
...
export const stockPriceTool: Tool = {
  ...
  async execute({ symbol, apiKey }: StockPriceParams, options: ToolExecutionOptions) {
    try {
      ...
      // Log error
      logger.error(LogCategory.NODE, '[StockPrice]', `Error fetching stock price for ${symbol}`, {
        error,
      });
