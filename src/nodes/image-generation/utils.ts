...
export async function generateImage(
  ...
  logger.debug(
    LogCategory.NODE,
    '[ImageGeneration]',
    `Executing ${inputImage ? 'image edit' : 'image generation'} for prompt: ${prompt}`,
  );
    ...
    if (inputImage) {
      ...
      try {
        ...
        if (estimatedSize > MAX_IMAGE_SIZE) {
          throw new Error(
            `Image is too large (${Math.round(estimatedSize / 1024 / 1024)}MB). Maximum size is 4MB.`,
          );
        }
        ...
        logger.error(LogCategory.NODE, '[ImageGeneration]', 'Error processing input image:', {
