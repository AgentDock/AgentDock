...
export async function getOrchestrationState(
  ...
  try {
    ...
    logger.error(
      LogCategory.API,
      'OrchestrationAdapter',
      'Error getting orchestration state',
      {
        error: error instanceof Error ? error.message : String(error),
        sessionId
      }
...
export async function trackToolUsage(
  ...
  try {
    ...
    logger.debug(
      LogCategory.API,
      'OrchestrationAdapter',
      'Tracked tool usage via core manager',
      { toolName, updatedState }
    );
    return updatedState;
  } catch (error) {
    logger.error(
...
export async function getAllowedTools(
  ...
  try {
    ...
    return await manager.getAllowedTools(
      config,
      llmMessages,
      sessionId,
      allTools
...
export async function addOrchestrationHeaders(
  ...
  try {
    ...
    logger.error(
      LogCategory.API,
      'OrchestrationAdapter',
      'Error adding orchestration headers',
      {
        error: error instanceof Error ? error.message : String(error),
        sessionId
      }
 ...
 */
export async function resetOrchestrationState(
  sessionId: SessionId
): Promise<void> {
  if (!sessionId) return;
      ...
      availableTools: step.availableTools
        ? {
            allowed: step.availableTools.allowed
              ? [...step.availableTools.allowed]
              : undefined,
            denied: step.availableTools.denied
              ? [...step.availableTools.denied]
              : undefined
          }
        : undefined
    }))
