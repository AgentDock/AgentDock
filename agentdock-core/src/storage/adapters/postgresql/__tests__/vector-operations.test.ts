...
const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://localhost/agentdock_test';
...
describe('Vector Operations Test', () => {
  ...
  test('should store and retrieve vectors', async () => {
    ...
    await memoryOps.batchCreateMemories([memory], [{
      memoryId: memory.id,
      vector: testVector
    }]);
  ...
  test('should handle vector similarity search with different thresholds', async () => {
    ...
    const memories = [
      ...
      },
      {
        ...
        vector: Array(1536).fill(0).map(() => Math.random()) // Very different
      }
    ];
