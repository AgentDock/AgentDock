...
export class SessionManager<T extends SessionState> {
  ...
  constructor(
    ...
    logger.debug(LogCategory.SESSION, 'SessionManager', 'Initialized with storage provider', {
      providerType: this.storage.constructor.name,
  ...
  async createSession(options: CreateSessionOptions = {}): Promise<SessionResult<T>> {
    try {
      ...
      logger.debug(LogCategory.SESSION, 'SessionManager', 'Created new session in storage', {
        sessionId,
        storageKey,
      });
      ...
      logger.error(LogCategory.SESSION, 'SessionManager', 'Error creating session', {
        error: errorMessage,
      });
    ...
    try {
      ...
      if (!storedData) {
        ...
        logger.debug(
          ...
          'Session not found in storage (expected for new session)',
          {
      ...
      if (
        storedData.metadata.lastAccessedAt instanceof Date &&
        ...
        logger.warn(
          LogCategory.SESSION,
          'SessionManager',
          'Invalid lastAccessedAt found, resetting.',
      ...
      logger.error(LogCategory.SESSION, 'SessionManager', 'Error getting session from storage', {
        sessionId,
        storageKey,
        error: errorMessage,
      });
      ...
      logger.warn(
        LogCategory.SESSION,
        'SessionManager.getSession',
        'Returning failure (error catch block)',
        {
          sessionId: sessionId?.substring(0, 8) + '...',
          error: errorMessage,
        },
  ...
  async updateSession(sessionId: SessionId, updateFn: (state: T) => T): Promise<SessionResult<T>> {
    ...
    try {
      ...
      await this.storage.set(storageKey, updatedSessionData, {
        ttlSeconds:
          updatedSessionData.ttlMs > 0 ? Math.floor(updatedSessionData.ttlMs / 1000) : undefined, // Use ttlSeconds
      });
      ...
      logger.error(LogCategory.SESSION, 'SessionManager', 'Error updating session in storage', {
        sessionId,
        storageKey,
        error: errorMessage,
      });
  ...
  async deleteSession(sessionId: SessionId): Promise<SessionResult<boolean>> {
    ...
    try {
      ...
      logger.debug(LogCategory.SESSION, 'SessionManager', 'Deleted session from storage', {
        sessionId,
        storageKey,
        deleted,
      });
      ...
      logger.error(LogCategory.SESSION, 'SessionManager', 'Error deleting session from storage', {
        sessionId,
        storageKey,
        error: errorMessage,
      });
