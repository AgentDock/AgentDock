...
export const ArxivSearchResultComponent: React.FC<{
  ...
  return (
    <div className="arxiv-search-result">
      ...
      {result.searchParams.category && <p>Category: {result.searchParams.category}</p>}
        ...
        {result.papers.map((paper, index) => (
          <li key={paper.id || index}>
            <div>
              ...
              {paper.categories.length > 0 && <p>Categories: {paper.categories.join(', ')}</p>}
              <p>Published: {paper.published}</p>
              {paper.updated !== paper.published && <p>Last Updated: {paper.updated}</p>}
              <p>
  ...
  return (
    <div className="arxiv-paper">
      ...
      {paper.updated !== paper.published && (
        <p>
          <strong>Last Updated:</strong> {paper.updated}
        </p>
      )}
