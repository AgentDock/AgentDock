...
describe('KeywordCoverageEvaluator', () => {
  const mockCriterion: EvaluationCriteria = {
    ...
    description: 'Test keyword coverage',
    scale: 'numeric',
  };
  ...
  describe('Constructor Error Handling', () => {
    it('should throw if criterionName is missing', () => {
      ...
      expect(() => new KeywordCoverageEvaluator(config as KeywordCoverageEvaluatorConfig)).toThrow(
        '[KeywordCoverageEvaluator] criterionName must be provided and non-empty.',
      );
    });

    it('should throw if criterionName is empty', () => {
      const config: Partial<KeywordCoverageEvaluatorConfig> = {
        criterionName: ' ',
        expectedKeywords: ['a'],
      };
      expect(() => new KeywordCoverageEvaluator(config as KeywordCoverageEvaluatorConfig)).toThrow(
    ...
    it('should throw if keywordsSourceField is config (or default) and expectedKeywords is missing/empty', () => {
      ...
      const config2: KeywordCoverageEvaluatorConfig = {
        criterionName: 'Test',
        expectedKeywords: [],
      }; // Empty expectedKeywords

      const expectedErrorMsg =
        "[KeywordCoverageEvaluator] expectedKeywords must be provided in config when keywordsSourceField is 'config' or default.";
      expect(() => new KeywordCoverageEvaluator(config1 as KeywordCoverageEvaluatorConfig)).toThrow(
        expectedErrorMsg,
      );
      expect(() => new KeywordCoverageEvaluator(config2)).toThrow(expectedErrorMsg);
    });

    it('should NOT throw if keywordsSourceField is not config and expectedKeywords is missing', () => {
      const config1: KeywordCoverageEvaluatorConfig = {
        criterionName: 'Test',
        keywordsSourceField: 'groundTruth',
      };
      const config2: KeywordCoverageEvaluatorConfig = {
        criterionName: 'Test',
        keywordsSourceField: 'context.someField',
      };
  ...
  describe('Core Coverage Calculation', () => {
    ...
    it('should be case-sensitive when configured', async () => {
      const caseSensitiveConfig: KeywordCoverageEvaluatorConfig = {
        ...config,
        caseSensitive: true,
      };
  ...
  describe('Keyword and Field Sourcing', () => {
    ...
    it('should source keywords from groundTruth (string) if configured', async () => {
      ...
      const input = createMockInput(
        'Response text with keyword1 and keyword2.',
        'keyword1 keyword2',
      );
      const results = await evaluator.evaluate(input, [mockCriterion]);
    ...
    it('should source keywords from groundTruth (array) if configured', async () => {
      ...
      const evaluator = new KeywordCoverageEvaluator(config);
      const input = createMockInput('Response text with keyword1 and keyword2.', [
        'keyword1',
        'keyword2',
      ]);
      const results = await evaluator.evaluate(input, [mockCriterion]);
    ...
    it('should source keywords from a context field (string) if configured', async () => {
      ...
      const evaluator = new KeywordCoverageEvaluator(config);
      const input = createMockInput('Response text with termA and termB.', undefined, {
        myKeywords: 'termA termB',
      });
      const results = await evaluator.evaluate(input, [mockCriterion]);
      expect(results[0].score).toBe(1);
      expect(results[0].reasoning).toContain('Found 2 out of 2 keywords. Coverage: 100.00%');
    ...
    it('should handle missing keywordsSourceField gracefully (returning error score)', async () => {
      ...
      expect(results[0].error).toContain(
        'Keywords source context.nonExistent not found or not a string/array.',
      );
    });
    ...
    it('should handle keywordsSourceField being of invalid type (returning error score)', async () => {
      ...
      expect(results[0].error).toContain(
        'Keywords source context.invalidType not found or not a string/array.',
      );
    });
    ...
    it('should use sourceTextField to get text from groundTruth', async () => {
      ...
      const evaluator = new KeywordCoverageEvaluator(config);
      const input = createMockInput('Any response', {
        details: { text: 'This is the key information.' },
      });
      const results = await evaluator.evaluate(input, [mockCriterion]);
      expect(results[0].score).toBe(1);
      expect(results[0].reasoning).toContain('Found 1 out of 1 keywords. Coverage: 100.00%');
    ...
    it('should use sourceTextField to get text from context', async () => {
      ...
      const evaluator = new KeywordCoverageEvaluator(config);
      const input = createMockInput('Any response', undefined, {
        data: { value: 'The target word is here.' },
      });
      const results = await evaluator.evaluate(input, [mockCriterion]);
      expect(results[0].score).toBe(1);
      expect(results[0].reasoning).toContain('Found 1 out of 1 keywords. Coverage: 100.00%');
    ...
    it('should handle missing sourceTextField gracefully (returning error score)', async () => {
      ...
      expect(results[0].reasoning).toContain(
        "Evaluation failed: Source text field 'context.nonExistent.path' did not yield a string or was not found.",
      );
      expect(results[0].error).toContain(
        'Source text field context.nonExistent.path not found in input or content is not a string.',
      );
    });
  });
