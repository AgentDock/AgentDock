...
const ChatDebug = dynamic(
  () => import('@/components/chat/chat-debug').then((mod) => mod.ChatDebug),
  {
    ssr: false,
    loading: () => null
  }
...
>(
  (
    ...
    const { messages: initialMessages, sessionId: initialSessionId } =
      React.useMemo(() => loadSavedData(), [loadSavedData]);
    ...
    const [orchestrationState, setOrchestrationState] =
      React.useState<OrchestrationState>({
    ...
    const {
      ...
      headers: {
        ...
        ...(orchestrationState.sessionId
          ? { 'x-session-id': orchestrationState.sessionId }
          : {})
      },
      body: {
      ...
      onResponse: async (response) => {
        ...
        const orchestrationHeader = response.headers.get(
        ...
        if (orchestrationHeader) {
          try {
            ...
            if (incomingState?.sessionId) {
              const incomingTools = Array.isArray(
                incomingState.recentlyUsedTools
              )
                ? incomingState.recentlyUsedTools
                : [];
              ...
              const hasStateChanged =
                ...
                JSON.stringify(incomingTools) !==
                  JSON.stringify(orchestrationState.recentlyUsedTools);

              if (hasStateChanged) {
                logDebug(
                  'ChatContainer',
                  'Orchestration state changed (via header), updating UI',
                  undefined,
                  {
                    from: {
                      step: orchestrationState.activeStep,
                      tools: orchestrationState.recentlyUsedTools
                    },
      ...
      onFinish: async (message) => {
        try {
          ...
          await logInfo(
            'ChatContainer',
            'Message processing complete (stream finished)',
            undefined,
            {
          ...
          await logError(
            'ChatContainer',
            'Failed to process message onFinish',
            error
          );
        }
      },
    ...
    const handleReset = React.useCallback(async () => {
      try {
        ...
        toast.error(
          'Failed to reset chat session. Please try reloading the page.'
        );
      }
    ...
    React.useEffect(() => {
      if (
        initialMessages.length > trimmedInitialMessages.length &&
        messages.length === trimmedInitialMessages.length
      ) {
    ...
    React.useEffect(() => {
      ...
      if (streamData?.orchestrationState?.sessionId) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.debug(
            '[ORCHESTRATION DEBUG] Received orchestration state via data:',
            JSON.stringify(streamData.orchestrationState, null, 2)
          );
        }

        const incomingState = streamData.orchestrationState;
        const incomingTools = Array.isArray(incomingState.recentlyUsedTools)
          ? incomingState.recentlyUsedTools
          : [];
        ...
        if (hasStateChanged) {
          logDebug(
            'ChatContainer',
            'Orchestration state changed (via data), updating UI',
            undefined,
            {
              from: {
                step: orchestrationState.activeStep,
          ...
          if (process.env.NODE_ENV === 'development') {
            console.debug(
              '[ORCHESTRATION DEBUG] Received state via data, but no change detected.'
            );
          }
        }
      }
    ...
    if (overlayErrorSource) {
      ...
      if (
        displayMessage?.includes('Overloaded') ||
        (overlayErrorSource as any).code === 'LLM_OVERLOADED_ERROR'
      ) {
        displayMessage =
      ...
      console.error(
        '[ChatContainer Error Debug] Raw error source:',
        overlayErrorSource
      );
      console.error(
        `[ChatContainer Error Debug] Message: "${overlayErrorSource.message}", Code: "${(overlayErrorSource as any).code}"`
      );
    ...
    return (
      <ErrorBoundary
        onError={(err) =>
          logError('ChatContainer', 'Global Error Boundary Caught:', err)
        }
