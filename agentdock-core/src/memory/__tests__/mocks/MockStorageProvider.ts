...
import { StorageProvider, MemoryOperations, MemoryData, MemoryOperationStats, MemoryRecallOptions } from '../../../storage/types';
...
export class MockStorageProvider implements StorageProvider {
  ...
  memory: MemoryOperations = {
    store: async (userId: string, agentId: string, memory: MemoryData): Promise<string> => {
      if (!userId?.trim()) throw new Error('userId is required for memory operations');
      if (!agentId?.trim()) throw new Error('agentId is required for memory operations');
    ...
    recall: async (userId: string, agentId: string, query: string, options?: MemoryRecallOptions): Promise<MemoryData[]> => {
      if (!userId?.trim()) throw new Error('userId is required for memory operations');
      if (!agentId?.trim()) throw new Error('agentId is required for memory operations');
      ...
      if (query?.trim()) {
        ...
        filtered = filtered.filter(m => 
          ...
          (m.metadata?.tags && Array.isArray(m.metadata.tags) && m.metadata.tags.some(t => t.toLowerCase().includes(lowerQuery)))
        );
      }
    ...
    update: async (userId: string, agentId: string, memoryId: string, updates: Partial<MemoryData>): Promise<void> => {
      if (!userId?.trim()) throw new Error('userId is required for memory operations');
    ...
    delete: async (userId: string, agentId: string, memoryId: string): Promise<void> => {
      if (!userId?.trim()) throw new Error('userId is required for memory operations');
    ...
    getById: async (userId: string, memoryId: string): Promise<MemoryData | null> => {
      if (!userId?.trim()) throw new Error('userId is required for memory operations');
    ...
    getStats: async (userId: string, agentId?: string): Promise<MemoryOperationStats> => {
      if (!userId?.trim()) throw new Error('userId is required for memory operations');
