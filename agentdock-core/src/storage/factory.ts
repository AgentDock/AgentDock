...
export class StorageFactory {
  ...
  private constructor() {
    ...
    logger.debug(LogCategory.STORAGE, 'StorageFactory', 'Initialized storage factory', {
  ...
  public registerProvider(type: string, factory: StorageProviderFactory): void {
    ...
    logger.debug(LogCategory.STORAGE, 'StorageFactory', 'Registered provider', { type });
  }
  ...
  public setDefaultType(type: string): void {
    ...
    logger.debug(LogCategory.STORAGE, 'StorageFactory', 'Set default provider type', { type });
  }
  ...
  public async clearCache(): Promise<void> {
    ...
    for (const [cacheKey, provider] of Object.entries(this.providerCache)) {
      try {
        ...
        logger.warn(LogCategory.STORAGE, 'StorageFactory', 'Error destroying provider', {
          cacheKey,
    ...
    logger.debug(LogCategory.STORAGE, 'StorageFactory', 'Cleared provider cache');
  }
}
...
export function createStorageProvider(config: {
  ...
  if (config.type === 'memory' && config.config?.isPersistent) {
    logger.debug(LogCategory.STORAGE, 'Factory', 'Creating persistent memory storage provider', {
