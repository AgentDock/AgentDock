...
import { AIOrchestrationState, OrchestrationConfig } from '../types/orchestration';
...
export class OrchestrationStateManager {
  ...
  public async getOrCreateState(
    ...
    if (!targetSessionId) {
      logger.warn(
        LogCategory.ORCHESTRATION,
        'OrchestrationStateManager',
        'getOrCreateState called without sessionId',
      );
      ...
      if (creationResult.success && creationResult.data) {
        ...
        logger.error(
          LogCategory.ORCHESTRATION,
          'OrchestrationStateManager',
          'Failed to create session for getOrCreateState',
  ...
  public async setActiveStep(
    ...
    logger.debug(LogCategory.ORCHESTRATION, 'OrchestrationStateManager', 'Setting active step', {
    ...
    if (result) {
      logger.debug(
        ...
        'Successfully updated active step in state',
        {
          ...
          recentlyUsedTools: result.recentlyUsedTools,
        },
      );
    } else {
      logger.error(
   ...
   */
  public async advanceSequence(sessionId: SessionId): Promise<OrchestrationState | null> {
    const state = await this.getState(sessionId);
    if (!state) return null;
  ...
  public async resetState(sessionId: SessionId): Promise<OrchestrationState | null> {
    ...
    logger.debug(LogCategory.ORCHESTRATION, 'OrchestrationStateManager', 'Reset state requested', {
      sessionId,
    });
    return null;
  }
