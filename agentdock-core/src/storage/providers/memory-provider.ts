...
export class MemoryStorageProvider implements StorageProvider {
  ...
  private startCleanup(intervalMs: number): void {
    ...
    this.cleanupInterval = setInterval(() => {
      try {
        ...
        logger.error(LogCategory.STORAGE, 'MemoryStorageProvider', 'Error during cleanup', {
  ...
  private cleanup(): void {
    ...
    if (removedCount > 0) {
      logger.debug(LogCategory.STORAGE, 'MemoryStorageProvider', 'Removed expired items', {
        namespace: this.namespace,
        count: removedCount,
   ...
   */
  public async setMany<T>(items: Record<string, T>, options: StorageOptions = {}): Promise<void> {
   ...
   */
  public async deleteMany(keys: string[], options: StorageOptions = {}): Promise<number> {
    let deletedCount = 0;
   ...
   */
  public async saveList<T>(key: string, values: T[], options: StorageOptions = {}): Promise<void> {
    const namespacedKey = this.getNamespacedKey(key, options.namespace);
    let expiresAt: number | undefined;
    if (typeof options.ttlSeconds === 'number' && options.ttlSeconds > 0) {
      expiresAt = Date.now() + options.ttlSeconds * 1000;
