...
const snowtracerSchema = z.object({
  ...
  address: z
    .string()
    .optional()
    .describe('Avalanche address to query (required for address-specific actions)'),
  ...
  apiKey: z
    .string()
    .optional()
    .describe('Optional Snowtrace API key (will use environment variable if not provided)'),
});
...
export const snowtracerTool: Tool = {
  ...
  async execute(
    { action, address, contractAddress, limit = 10, apiKey }: SnowtracerParams,
    ...
    try {
      ...
      if (
        ['address_balance', 'transactions', 'token_transfers', 'contract_abi'].includes(action) &&
        !address
      ) {
      ...
      // Log error
      logger.error(LogCategory.NODE, '[Snowtracer]', `Error executing action: ${action}`, {
        error,
      });
...
async function getTransactions(address: string, limit: number, apiKey?: string) {
  const transactions = await snowtraceAPI.account.getTransactions(
    address,
    0,
    99999999,
    1,
...
async function getTokenTransfers(
  ...
  const transfers = await snowtraceAPI.account.getTokenTransfers(
    address,
    contractAddress,
    0,
    99999999,
