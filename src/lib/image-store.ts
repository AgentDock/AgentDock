...
if (!global.imageStoreGlobal) {
  ...
  logger.info(LogCategory.SYSTEM, 'ImageStore', 'Initialized global image store');
}
...
export function storeImage(id: string, image: StoredImage): void {
  ...
  logger.debug(LogCategory.SYSTEM, 'ImageStore', `Image stored with ID: ${id}`, {
...
export function getStoredImage(id: string): StoredImage | undefined {
  ...
  logger.debug(LogCategory.SYSTEM, 'ImageStore', `Retrieving image with ID: ${id}`, {
    found: !!image,
    storeSize: imageStore.size,
  });
...
export function clearMemoryStore(): void {
  ...
  imageStore.clear();
  logger.info(LogCategory.SYSTEM, 'ImageStore', 'Cleared global image store via function call', {
    initialSize,
    finalSize: imageStore.size,
  });
}
...
export async function storeAndGetImageUrl(
  ...
  if (USE_BLOB_STORAGE) {
    ...
    try {
      ...
      logger.info(LogCategory.SYSTEM, 'ImageStore', `Vercel Blob upload successful`, {
        url: blobResult.url,
      });
...
export async function getImageDataFromUrl(url: string): Promise<string | null> {
  try {
    ...
    if (!imageId) {
      logger.warn(LogCategory.SYSTEM, 'ImageStore', `Failed to extract image ID from URL: ${url}`);
      return null;
    }
    ...
    if (!response.ok) {
      logger.error(LogCategory.SYSTEM, 'ImageStore', `Failed to fetch image from API: ${apiUrl}`, {
    ...
    logger.error(LogCategory.SYSTEM, 'ImageStore', `Error getting image data from URL: ${url}`, {
      error: error instanceof Error ? error.message : String(error),
    });
    return null;
  }
