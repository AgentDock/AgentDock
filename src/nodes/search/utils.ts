...
export async function searchWeb(query: string, limit: number = 5): Promise<SearchResult[]> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error(LogCategory.NODE, '[SearchAPI]', `API error: ${response.status}`, {
        error: errorText,
      });
    ...
    if (data.answerBox && (data.answerBox.answer || data.answerBox.snippet)) {
      results.push({
        ...
        url: '#',
        snippet: cleanText(
          data.answerBox.answer || data.answerBox.snippet || 'No description available.',
        ),
        isFeatured: true,
      });
    }
    ...
    if (results.length === 0) {
      logger.warn(LogCategory.NODE, '[SearchAPI]', 'No valid search results found for query', {
        query,
      });
      results.push({
