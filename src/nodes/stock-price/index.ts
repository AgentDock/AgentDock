...
const stockPriceSchema = z.object({
  symbol: z
    .string()
    .describe('Stock symbol to look up price for (e.g., AAPL, MSFT, GOOGL)'),
  apiKey: z
    .string()
    .optional()
    .describe(
      'Optional AlphaVantage API key (will use environment variable if not provided)'
    )
});
  ...
  async execute({ symbol, apiKey }: StockPriceParams) {
    try {
      ...
      logger.debug(
        LogCategory.NODE,
        '[StockPrice]',
        `Fetching stock price for ${formattedSymbol}`
      );
      ...
      logger.debug(
        LogCategory.NODE,
        '[StockPrice]',
        `Successfully fetched stock price for ${formattedSymbol}`,
        {
