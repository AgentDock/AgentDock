...
describe('WorkingMemory - Actual Implementation', () => {
  ...
  describe('Configuration Requirements (NO DEFAULTS)', () => {
    test('requires storage with memory operations', () => {
      ...
      expect(() => new WorkingMemory(storageWithoutMemory, testConfig.memory.working))
    ...
    test('uses ttlSeconds from configuration for expiration', async () => {
      ...
      const memoryId = await workingMemory.store(userId, agentId, 'Test working memory content');
      ...
      const expectedExpiry = stored!.createdAt + (testConfig.memory.working.ttlSeconds * 1000);
    ...
    test('uses maxContextItems from configuration', async () => {
      ...
      expect(stored?.metadata?.contextWindow).toBe(testConfig.memory.working.maxContextItems);
    });
  });
  ...
  describe('User Isolation - CRITICAL SECURITY', () => {
    ...
    test('enforces user isolation on recall operations', async () => {
      ...
      await workingMemory.store(testConfig.users.alice, testConfig.agents.shared, 'Alice working on project Alpha');
      await workingMemory.store(testConfig.users.bob, testConfig.agents.shared, 'Bob working on project Beta');
      
      // Recall should only return user's own memories
      const aliceMemories = await workingMemory.recall(testConfig.users.alice, testConfig.agents.shared, 'project');
      const bobMemories = await workingMemory.recall(testConfig.users.bob, testConfig.agents.shared, 'project');
    ...
    test('requires userId for ALL operations', async () => {
      ...
      await expect(workingMemory.store('', agentId, 'content'))
        .rejects.toThrow('userId must be a non-empty string for working memory operations');
      
      await expect(workingMemory.recall('', agentId, 'query'))
        .rejects.toThrow('userId must be a non-empty string for working memory operations');
      
      await expect(workingMemory.clear('', agentId))
        .rejects.toThrow('userId must be a non-empty string for working memory operations');
      
      await expect(workingMemory.getStats(''))
        .rejects.toThrow('userId is required for working memory operations');
      
      await expect(workingMemory.getById('', 'memoryId'))
        .rejects.toThrow('userId is required for working memory operations');
    });
  });
  ...
  describe('Storage Delegation - Core Architecture', () => {
    ...
    test('delegates getById operations to storage layer', async () => {
      ...
      const userId = testConfig.users.alice;
      const memoryId = await workingMemory.store(userId, testConfig.agents.shared, 'Test content');
  ...
  describe('TTL and Expiration Management', () => {
    test('stores memories with correct TTL expiration', async () => {
      ...
      const memoryId = await workingMemory.store(userId, agentId, 'TTL test memory', {
    ...
    test('uses default TTL when not specified', async () => {
      ...
      const memoryId = await workingMemory.store(userId, agentId, 'Default TTL memory');
  ...
  describe('Session Management', () => {
    test('stores custom sessionId when provided', async () => {
      ...
      const memoryId = await workingMemory.store(userId, agentId, 'Session test', { sessionId });
    ...
    test('generates sessionId automatically when not provided', async () => {
      ...
      const memoryId = await workingMemory.store(userId, agentId, 'Auto session test');
    ...
    test('clear() removes all memories for user/agent', async () => {
      ...
      await workingMemory.store(userId, agentId, 'Memory 1', { sessionId: 'session-1' });
      await workingMemory.store(userId, agentId, 'Memory 2', { sessionId: 'session-2' });
      await workingMemory.store(userId, agentId, 'Memory 3', { sessionId: 'session-1' });
  ...
  describe('BaseMemoryType Integration', () => {
    test('inherits automatic connection discovery from BaseMemoryType', async () => {
      ...
      const memoryId = await workingMemory.store(userId, agentId, 'Content for connection test');
  ...
  describe('Error Handling', () => {
    test('handles storage errors gracefully', async () => {
      ...
      await expect(workingMemory.store(testConfig.users.alice, testConfig.agents.shared, 'Test'))
        .rejects.toThrow('Storage failure');
    });

    test('handles missing memory gracefully on getById', async () => {
      const result = await workingMemory.getById(testConfig.users.alice, 'non-existent-id');
      expect(result).toBeNull();
    });
  });
}); 