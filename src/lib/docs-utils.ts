    ...
    if (stats.isDirectory()) {
      ...
      for (const dir of directories) {
        ...
        if (dirItems.length > 0) {
          ...
          const url = hasReadme ? `/docs/${relativePath.split(path.sep).join('/')}` : undefined;
      ...
      const title =
        extractTitle(content) || formatTitle(path.basename(basePath).replace('.md', ''));
...
export function mapSectionToSidebar(section: DocSection): SidebarSection {
  ...
  if (section.basePath) {
    ...
    const convertedItems = autoItems
      .map((item) => {
        if (item.url) {
          return {
            title: item.title,
