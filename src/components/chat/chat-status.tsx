...
export function ChatError({
  ...
  const errorTitle = isMissingApiKey
    ? 'API Key Required'
    : isRateLimitError
      ? 'Rate Limit Exceeded'
  ...
  return (
    <div
      className={cn(
        ...
        isOverlay ? 'absolute inset-0 z-50 bg-background/80 backdrop-blur-sm' : 'h-full',
      )}
    >
      <Card className="max-w-md w-full shadow-lg border border-border/50">
        ...
        <CardContent className="pt-0 pb-4">
          <p className="text-sm text-card-foreground leading-relaxed">{friendlyMessage}</p>
        ...
        <CardFooter className="flex-col items-start pt-4">{getActionContent()}</CardFooter>
      </Card>
    </div>
  );
}
