...
- **Cerebras AI**: Now supports the Cerebras AI provider with models like llama3.1-8b, llama-3.3-70b, and llama-4-scout. API keys in both `csk-` and `csk_` formats are supported.

### Improvements

- **Enhanced Error Handling**: Better error handling and logging across all LLM providers.
- **Dynamic Model Fetching**: LLM adapters now dynamically fetch available models from provider APIs when possible.
- **Improved Testing**: Added comprehensive test utilities for verifying provider integrations.
...
CEREBRAS_API_KEY=csk-xxxxxxx      # Cerebras AI API key
