...
export class VercelKVProvider implements StorageProvider {
  ...
  constructor(config: VercelKVConfig = {}) {
    ...
    logger.debug(LogCategory.STORAGE, 'VercelKVProvider', 'Initialized Vercel KV provider', {
  ...
  async get<T>(key: string, options?: StorageOptions): Promise<T | null> {
    ...
    try {
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error getting value', {
        key: fullKey,
  ...
  async set<T>(key: string, value: T, options?: StorageOptions): Promise<void> {
    ...
    try {
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error setting value', {
        key: fullKey,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async delete(key: string, options?: StorageOptions): Promise<boolean> {
    ...
    try {
      ...
      logger.debug(LogCategory.STORAGE, 'VercelKVProvider', '[DELETE]', { key: fullKey, deleted });

      return deleted;
    } catch (error) {
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error deleting value', {
        key: fullKey,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async exists(key: string, options?: StorageOptions): Promise<boolean> {
    ...
    try {
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error checking existence', {
        key: fullKey,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async getMany<T>(keys: string[], options?: StorageOptions): Promise<Record<string, T | null>> {
    ...
    try {
      ...
      return Object.fromEntries(keys.map((key, i) => [key, values[i] as T | null]));
    } catch (error) {
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error getting multiple values', {
        keys: fullKeys,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async setMany<T>(items: Record<string, T>, options?: StorageOptions): Promise<void> {
    const entries = Object.entries(items).map(([key, value]) => [this.getKey(key, options), value]);
    ...
    try {
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error setting multiple values', {
        keys: entries.map(([key]) => key),
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async deleteMany(keys: string[], options?: StorageOptions): Promise<number> {
    const fullKeys = keys.map((key) => this.getKey(key, options));
    try {
      const results = await Promise.all(fullKeys.map((key) => this.client.del(key)));
      const deleted = results.filter((result: number) => result === 1).length;

      logger.debug(LogCategory.STORAGE, 'VercelKVProvider', '[DELETE_MANY]', {
        keys: fullKeys,
        deleted,
      });
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error deleting multiple values', {
        keys: fullKeys,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async list(prefix: string, options?: ListOptions): Promise<string[]> {
    ...
    try {
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error listing keys', {
        prefix: fullPrefix,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async clear(prefix?: string): Promise<void> {
    try {
      const searchPrefix = prefix ? this.getKey(prefix) : `${this.namespace}:`;
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error clearing keys', {
  ...
  async getList<T>(
    ...
    try {
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error getting list', {
        key: fullKey,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async saveList<T>(key: string, values: T[], options?: StorageOptions): Promise<void> {
    ...
    try {
      ...
      logger.error(LogCategory.STORAGE, 'VercelKVProvider', 'Error saving list', {
        key: fullKey,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error; // Re-throw to indicate failure
  ...
  async destroy(): Promise<void> {
    ...
    logger.debug(LogCategory.STORAGE, 'VercelKVProvider', 'Vercel KV provider destroyed');
  }
}
