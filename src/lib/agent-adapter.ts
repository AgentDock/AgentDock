...
export function initializeAdapter() {
  ...
  try {
    ...
    logger.error(LogCategory.API, 'AgentAdapter', 'Error initializing adapter', {
...
export async function processAgentMessage(options: HandleMessageOptions) {
  ...
  logger.debug(LogCategory.API, 'AgentAdapter', `Processing agent message for ${agentId}`, {
    provider,
  ...
  if (!sessionId) {
    logger.warn(LogCategory.API, 'AgentAdapter', 'No session ID provided, generated:', {
      finalSessionId,
    });
  }
  ...
  try {
    ...
    logger.debug(LogCategory.API, 'AgentAdapter', 'ensureStateExists call completed', {
