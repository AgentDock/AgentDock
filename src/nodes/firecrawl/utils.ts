...
class FirecrawlClient {
  ...
  private async request(endpoint: string, method: string = 'POST', body?: any): Promise<any> {
    ...
    if (!response.ok) {
      ...
      logger.error(LogCategory.NODE, '[FirecrawlAPI]', `API error: ${response.status}`, {
        error: errorText,
      });
...
export async function searchFirecrawl(
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      for (const item of response.data) {
        ...
        results.push({
          ...
          snippet:
            cleanText(
              item.description || (item.markdown ? item.markdown.substring(0, 200) + '...' : ''),
    ...
    if (results.length === 0) {
      logger.warn(LogCategory.NODE, '[FirecrawlAPI]', 'No valid search results found for query', {
        query,
      });
      results.push({
