...
describe('ToolUsageEvaluator', () => {
  ...
  describe('Constructor Error Handling', () => {
    it('should throw if config.rules is missing or empty', () => {
      ...
      expect(() => new ToolUsageEvaluator(badConfig1 as ToolUsageEvaluatorConfig)).toThrow(
    ...
    it('should throw if a rule has empty criterionName', () => {
      const rules = [
        { ...validRuleBase, criterionName: ' ', expectedToolName: 'get_weather' },
      ] as ToolUsageRule[];
      expect(() => new ToolUsageEvaluator({ rules })).toThrow(
  ...
  describe('Multiple Rules', () => {
    ...
    it('should evaluate multiple rules correctly (one required fail, one optional pass)', async () => {
      ...
      expect(results.find((r) => r.criterionName === 'UsedWeather')?.reasoning).toContain(
        'was not called',
      );
      expect(results.find((r) => r.criterionName === 'UsedStock')?.score).toBe(true);
      expect(results.find((r) => r.criterionName === 'UsedStock')?.reasoning).toContain(
        'was called',
      );
    });
    ...
    it('should evaluate multiple rules correctly (one required pass, one optional missing)', async () => {
      ...
      expect(results.find((r) => r.criterionName === 'UsedStock')?.reasoning).toContain(
        'was not called. (Tool was optional)',
      );
    });
    ...
    it('should evaluate multiple rules including argument checks', async () => {
      ...
      expect(results.find((r) => r.criterionName === 'UsedStock')?.reasoning).toContain(
        'Argument check failed for the first call: Bad ticker',
      );
