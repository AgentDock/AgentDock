...
import { loadHistoryAction, saveHistoryAction, clearHistoryAction } from './history-actions';
...
function ImageGenerationWithParams(props: { editIdFromUrl?: string | null }) {
  ...
  useEffect(() => {
    ...
    if (!storedSessionId) {
      ...
      logger.debug(LogCategory.SYSTEM, 'ImageGenPage', 'Generated new imageGenSessionId', {
        sessionId: storedSessionId,
      });
    } else {
      logger.debug(LogCategory.SYSTEM, 'ImageGenPage', 'Retrieved existing imageGenSessionId', {
        sessionId: storedSessionId,
      });
    }
      ...
      const fetchStoredImagePaths = async () => {
        try {
          ...
          if (response.ok) {
            ...
            logger.debug(LogCategory.SYSTEM, 'ImageGenPage', 'Loaded gallery paths from API', {
  ...
  const handlePromptSubmit = async (promptText: string) => {
    ...
    try {
      ...
      if (result?.image) {
        ...
        logger.debug(LogCategory.SYSTEM, 'ImageGenPage', 'History before saving via action:', {
          finalHistory,
        });
        ...
        if (returnedImageUrl.startsWith('http')) {
          ...
          logger.debug(LogCategory.SYSTEM, 'ImageGenPage', 'Updated localStorage gallery (hub)', {
          ...
          logger.debug(LogCategory.SYSTEM, 'ImageGenPage', 'Updated memory gallery (local)', {
      ...
      const message =
        actionError instanceof Error ? actionError.message : 'Failed to generate/save history.';
      setError(message);
    } finally {
      setLoading(false);
    }
              ...
              <div className="flex items-center justify-center h-80 rounded-lg bg-gradient-to-br from-primary/5 to-accent/10 animate-pulse">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="text-center">
                    <p className="text-base font-medium text-foreground">Creating your image...</p>
      ...
      <div id="image-gallery" className="mt-16 scroll-mt-4 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Browse and edit your images</p>
          </div>
