...
_Note: AgentDock Core is currently under active development (pre-release) and will be published as a versioned NPM package upon reaching a stable release._
...
_Note: The example above is simplified. Full usage, especially with sessions and orchestration, requires integrating `SessionManager` and `OrchestrationManager` which depend on the chosen storage provider._
