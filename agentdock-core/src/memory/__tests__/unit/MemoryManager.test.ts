...
describe('MemoryManager - Core Functionality', () => {
  ...
  describe('Configuration Requirements (NO DEFAULTS)', () => {
    test('requires working memory configuration', () => {
      expect(() => new MemoryManager(storage, {}))
        .toThrow('Working memory configuration is required');
    });

    test('requires all memory type configurations', () => {
      expect(() => new MemoryManager(storage, { working: testConfig.memory.working }))
  ...
  describe('User Isolation - CRITICAL SECURITY TEST', () => {
    ...
    test('REQUIRES userId for ALL operations', async () => {
      await expect(memoryManager.store('', testConfig.agents.shared, 'content'))
        .rejects.toThrow('userId must be a non-empty string for memory operations');
      
      await expect(memoryManager.recall('', testConfig.agents.shared, 'query'))
        .rejects.toThrow('userId must be a non-empty string for memory operations');
      
      await expect(memoryManager.getStats(''))
        .rejects.toThrow('userId must be a non-empty string for memory operations');
    });
    ...
    test('cross-agent isolation within same user', async () => {
      ...
      await memoryManager.store(userId, testConfig.agents.personal, 'Personal agent data');
      await memoryManager.store(userId, testConfig.agents.shared, 'Shared agent data');
      
      // Recall should be agent-specific
      const personalMemories = await memoryManager.recall(userId, testConfig.agents.personal, 'data');
      const sharedMemories = await memoryManager.recall(userId, testConfig.agents.shared, 'data');
  ...
  describe('Memory Type Delegation', () => {
    test('delegates to correct memory type', async () => {
      ...
      const workingId = await memoryManager.store(userId, agentId, 'Working', MemoryType.WORKING);
      const episodicId = await memoryManager.store(userId, agentId, 'Episodic', MemoryType.EPISODIC);
      const semanticId = await memoryManager.store(userId, agentId, 'Semantic', MemoryType.SEMANTIC);
      const proceduralId = await memoryManager.store(userId, agentId, 'Procedural', MemoryType.PROCEDURAL);
  ...
  describe('Basic Operations Validation', () => {
    test('requires agentId and content for store operations', async () => {
      ...
      await expect(memoryManager.store(userId, '', 'content'))
        .rejects.toThrow('Agent ID and content are required');
      
      await expect(memoryManager.store(userId, testConfig.agents.shared, ''))
        .rejects.toThrow('Agent ID and content are required');
    ...
    test('requires agentId and query for recall operations', async () => {
      ...
      await expect(memoryManager.recall(userId, '', 'query'))
        .rejects.toThrow('Agent ID and query are required');
      
      await expect(memoryManager.recall(userId, testConfig.agents.shared, ''))
        .rejects.toThrow('Agent ID and query are required');
    });
    ...
    test('getStats works with and without agentId', async () => {
      ...
      await memoryManager.store(userId, testConfig.agents.personal, 'Personal memory');
      await memoryManager.store(userId, testConfig.agents.shared, 'Shared memory');
      ...
      const agentStats = await memoryManager.getStats(userId, testConfig.agents.personal);
  ...
  describe('Real-world Usage Scenarios', () => {
    test('handles concurrent operations for different users', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        operations.push(
          memoryManager.store(testConfig.users.alice, testConfig.agents.shared, `Alice memory ${i}`),
          memoryManager.store(testConfig.users.bob, testConfig.agents.shared, `Bob memory ${i}`)
        );
      }
      ...
      const aliceMemories = await memoryManager.recall(testConfig.users.alice, testConfig.agents.shared, 'memory');
      const bobMemories = await memoryManager.recall(testConfig.users.bob, testConfig.agents.shared, 'memory');
      
      expect(aliceMemories.every(m => m.content.includes('Alice'))).toBe(true);
