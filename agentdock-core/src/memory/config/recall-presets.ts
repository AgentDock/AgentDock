...
export function validateHybridWeights(
  ...
  if (weights.text < 0.25) {
    console.warn(
      `⚠️  Text weight (${weights.text}) below 0.25 may cause catastrophic failures ` +
        `on specialized domains (medical, legal, technical). BEIR research recommends minimum 0.25 for safety.`
    );
  }

  if (weights.vector < 0.2) {
    console.warn(
      `⚠️  Vector weight (${weights.vector}) below 0.2 may significantly reduce ` +
  ...
  if (weights.temporal < 0.1) {
    console.warn(
      `⚠️  Temporal weight (${weights.temporal}) below 0.1 may reduce conversation ` +
        `continuity in multi-turn interactions.`
    );
  }
}
