  ...
  async checkPerformanceAlerts(
    ...
    try {
      ...
      if (
        ...
        alerts.push(
          `High processing time: ${metrics.processingTimeMs}ms (threshold: ${thresholds.maxProcessingTime}ms)`
        );
      }
      ...
      if (
        ...
        metrics.efficiency < thresholds.minEfficiency
      ) {
        alerts.push(
          `Low efficiency: ${metrics.efficiency.toFixed(3)} (threshold: ${thresholds.minEfficiency})`
        );
      }
      ...
      if (thresholds.maxCost && metrics.costUsd > thresholds.maxCost) {
        alerts.push(
          `High cost: $${metrics.costUsd.toFixed(4)} (threshold: $${thresholds.maxCost})`
        );
      }
