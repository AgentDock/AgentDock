...
export class AgentNode extends BaseNode<AgentNodeConfig> {
  ...
  constructor(id: string, config: AgentNodeConfig) {
    ...
    if (!config.agentConfig) {
      logger.warn(
        LogCategory.NODE,
        'AgentNode',
        'AgentConfig missing during construction, using minimal default.',
        { nodeId: id },
      );
      config.agentConfig = {
        version: '1.0',
    ...
    if (config.fallbackApiKey && (config.fallbackProvider || config.fallbackModel)) {
      ...
      if (
        config.fallbackApiKey === config.apiKey &&
        fallbackProviderOrDefault === this.llm.getProvider() &&
        fallbackModelOrDefault === this.llm.getModelId()
      ) {
        logger.warn(
          LogCategory.NODE,
          'AgentNode',
          'Fallback LLM configuration is identical to primary LLM.',
          { nodeId: this.id },
        );
      } else {
  ...
  private createLLMInstance(
    ...
    logger.debug(LogCategory.NODE, 'AgentNode', 'Entering createLLMInstance', {
      nodeId: this.id,
      provider,
    });
    try {
      ...
      logger.debug(LogCategory.NODE, 'AgentNode', 'Attempting to derive provider', {
        nodeId: this.id,
        initialProvider: determinedProvider,
      });
      if (!determinedProvider && this.config.agentConfig) {
        ...
        if (!determinedProvider) {
          logger.warn(
            LogCategory.NODE,
            'AgentNode',
            'Could not determine LLM provider from agentConfig nodes.',
            { nodeId: this.id },
          );
        ...
        logger.warn(
          LogCategory.NODE,
          'AgentNode',
          'No provider specified and could not derive from agentConfig. Using default.',
          { nodeId: this.id },
        );
        determinedProvider = 'anthropic';
      }
      logger.debug(LogCategory.NODE, 'AgentNode', 'Derived provider', {
        nodeId: this.id,
        determinedProvider,
      });

      logger.debug(LogCategory.NODE, 'AgentNode', 'Attempting to derive model', {
        nodeId: this.id,
      ...
      if (!modelName) {
        logger.error(
          LogCategory.NODE,
          'AgentNode',
          'Could not determine model name from options, agentConfig, or provider defaults.',
          { nodeId: this.id, provider: determinedProvider },
        );
        throw createError(
          'node',
          `Failed to determine model name for provider ${determinedProvider}. Please configure explicitly.`,
          ErrorCode.NODE_INITIALIZATION,
        );
      }
      logger.debug(LogCategory.NODE, 'AgentNode', 'Derived model', { nodeId: this.id, modelName });

      logger.debug(LogCategory.NODE, 'AgentNode', 'Calling actual createLLM function', {
        nodeId: this.id,
        provider: determinedProvider,
        model: modelName,
      });
      ...
      throw createError(
        'node',
        `Failed to create LLM for provider ${provider ?? 'unknown'}`,
        ErrorCode.NODE_INITIALIZATION,
        { cause: error },
      );
    }
  ...
  private async getAvailableTools(
    ...
    try {
      ...
      const allAgentToolsList: Tool[] = Object.entries(allAgentToolsObj).map(([toolKey, tool]) => ({
        ...tool,
        name: toolKey,
      }));
      ...
      if (orchestrationConfig.steps.length > 0) {
        ...
        allowedToolNames = await orchestrationManager.getAllowedTools(
          orchestrationConfig,
          llmMessages,
          sessionId,
          allToolIds,
      ...
      const availableTools = allAgentToolsList.filter((tool) =>
        allowedToolNames.includes(tool.name),
      );
      logger.debug(LogCategory.NODE, 'AgentNode', 'Tools available for turn', {
        nodeId: this.id,
      ...
      logger.error(LogCategory.NODE, 'AgentNode', 'Error getting available tools', {
        nodeId: this.id,
        sessionId: sessionId?.substring(0, 8),
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async handleMessage(
    ...
    if (!activeLLM) {
      throw createError(
        'node',
        'No active LLM available for handleMessage',
    ...
    try {
      ...
      if (!currentOrchestrationState) {
        logger.debug(LogCategory.NODE, 'AgentNode', 'Fetching orchestration state for prompt', {
          nodeId: this.id,
          sessionId: sessionId?.substring(0, 8),
        });
      ...
      const dynamicStateForPrompt: DynamicOrchestrationState = {
        ...
        recentlyUsedTools:
          ...
          currentOrchestrationState.recentlyUsedTools.length > 0
            ? currentOrchestrationState.recentlyUsedTools
            : undefined,
      };
      logger.debug(LogCategory.NODE, 'AgentNode', 'Dynamic state prepared for prompt', {
        nodeId: this.id,
        sessionId: sessionId?.substring(0, 8),
        dynamicState: dynamicStateForPrompt,
      });
...
ISO timestamp: ${now.toISOString()}`;
      ...
      const maxSteps =
        runtimeOverrides?.maxSteps ??
        (typeof this.config.agentConfig?.options?.maxSteps === 'number'
          ? this.config.agentConfig.options.maxSteps
          : undefined) ??
        5;
      ...
      logger.debug(LogCategory.NODE, 'AgentNode', 'Returning stream result object from service', {
        nodeId: this.id,
        sessionId: sessionId?.substring(0, 8),
      });
      return result as any;
    } catch (error) {
      logger.error(LogCategory.NODE, 'AgentNode', 'Error in handleMessage', {
        nodeId: this.id,
        sessionId: sessionId?.substring(0, 8),
        error: error instanceof Error ? error.message : String(error),
  ...
  async execute(input: unknown): Promise<unknown> {
    logger.warn(
      LogCategory.NODE,
      'AgentNode',
      'Direct execute() lacks context. Use handleMessage.',
      { nodeId: this.id },
    );
    throw createError(
      'node',
      'Direct execute() not supported for AgentNode. Use handleMessage.',
      ErrorCode.NODE_EXECUTION,
    );
  }
  ...
  protected getDescription(): string {
    return (
      this.config?.agentConfig?.description || 'Core Agent Node using Vercel AI SDK streamText.'
    );
  }
  ...
  protected getOutputs(): readonly NodePort[] {
    return [
      {
        id: 'streamResult',
        type: 'AgentDockStreamResult<Record<string, CoreTool>>',
        label: 'Stream Result',
        required: true,
      },
    ];
