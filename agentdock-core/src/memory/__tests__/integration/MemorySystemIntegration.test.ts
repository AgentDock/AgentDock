...
describe('Memory System Integration - All Memory Types Working Together', () => {
  ...
  describe('Cross-Memory Type Workflows', () => {
    test('realistic learning workflow: Working -> Episodic -> Semantic -> Procedural', async () => {
      ...
      const workingId = await workingMemory.store(userId, agentId, 'Learning about TypeScript interfaces', {
        sessionId,
        importance: 0.8
      });
      
      // 2. Convert to episodic memory (experience)
      const episodicId = await episodicMemory.store(userId, agentId, 'Successfully implemented TypeScript interfaces in project', {
        tags: ['typescript', 'learning', 'project'],
        importance: 0.9
      });
      
      // 3. Extract semantic knowledge
      const semanticId = await semanticMemory.store(userId, agentId, 'TypeScript interfaces define object structure contracts', {
      ...
      const retrievedEpisodic = await episodicMemory.getById(userId, episodicId);
      const retrievedSemantic = await semanticMemory.getById(userId, semanticId);
      const retrievedProcedural = await proceduralMemory.getById(userId, proceduralResult.patternId);
    ...
    test('information flows from temporary to permanent storage', async () => {
      ...
      await workingMemory.store(userId, agentId, 'Currently debugging authentication issue', {
        sessionId: 'debug_session',
        importance: 0.7
      });
      
      // Episodic memory: experience record
      await episodicMemory.store(userId, agentId, 'Fixed authentication by updating JWT secret', {
        tags: ['debugging', 'authentication', 'jwt'],
        importance: 0.8
      });
      
      // Semantic memory: extracted knowledge
      await semanticMemory.store(userId, agentId, 'JWT authentication requires consistent secret across services', {
        keywords: ['jwt', 'authentication', 'microservices'],
        confidence: 0.95
      });
      
      // Procedural memory: reusable pattern
      await proceduralMemory.learn(userId, agentId, 'authentication failing', 'check JWT secret consistency');
      
      // Verify information persistence increases from working to procedural
      const workingMemories = await workingMemory.recall(userId, agentId, 'authentication', 10);
      const episodicMemories = await episodicMemory.recall(userId, agentId, 'authentication', { limit: 10 });
      const semanticResults = await semanticMemory.search(userId, agentId, 'authentication');
      const proceduralResults = await proceduralMemory.getRecommendedActions(userId, agentId, 'authentication failing');
  ...
  describe('Storage Layer Consistency', () => {
    ...
    test('user isolation works across all memory types', async () => {
      ...
      const aliceWorking = await workingMemory.recall(aliceId, agentId, 'memory', 10);
      const bobWorking = await workingMemory.recall(bobId, agentId, 'memory', 10);
  ...
  describe('Configuration Consistency', () => {
    ...
    test('all memory types respect their configurations', async () => {
      ...
      const episodicId = await episodicMemory.store(userId, agentId, 'Decay test');
      ...
      const semanticId = await semanticMemory.store(userId, agentId, 'Confidence test');
      const semanticStored = await storage.memory.getById!(userId, semanticId);
      expect(semanticStored?.metadata?.confidence).toBe(testConfig.memory.semantic.confidenceThreshold);
      
      // Procedural memory should respect pattern thresholds
      const proceduralResult = await proceduralMemory.learn(userId, agentId, 'test trigger', 'test action');
      expect(proceduralResult.confidence).toBe(testConfig.memory.procedural.confidenceThreshold);
    });
  });
  ...
  describe('Complex Multi-Memory Scenarios', () => {
    test('debugging workflow uses all memory types', async () => {
      ...
      await workingMemory.store(userId, agentId, 'Investigating 500 error in user registration API', {
        sessionId,
        contextWindow: 5
      });
      
      // 2. Episodic memory: what happened during debugging
      await episodicMemory.store(userId, agentId, 'Found database connection timeout causing 500 errors', {
        tags: ['debugging', 'database', 'timeout', '500-error'],
        importance: 0.9
      });
      
      // 3. Semantic memory: knowledge extracted
      await semanticMemory.store(userId, agentId, 'Database connection timeouts manifest as 500 errors in APIs', {
      ...
      await proceduralMemory.learn(userId, agentId, 'API returning 500 errors', 'check database connection timeout');
      
      // Simulate future debugging: procedural memory should suggest the pattern
      const recommendations = await proceduralMemory.getRecommendedActions(userId, agentId, 'API returning 500');
      expect(recommendations).toHaveLength(1);
      expect(recommendations[0].pattern.content).toContain('database connection timeout');
      
      // Semantic memory should provide related knowledge
      const relatedKnowledge = await semanticMemory.search(userId, agentId, '500 error');
      expect(relatedKnowledge).toHaveLength(1);
      expect(relatedKnowledge[0].content).toContain('Database connection timeouts');
    });
    ...
    test('learning progression across memory types', async () => {
      ...
      await workingMemory.store(userId, agentId, 'Reading React documentation about hooks');
      
      // Day 2: Episodic memory - practical experience  
      await episodicMemory.store(userId, agentId, 'Built first React component using useState hook', {
        tags: ['react', 'hooks', 'useState', 'component']
      });
      
      // Day 3: More experience
      await episodicMemory.store(userId, agentId, 'Successfully implemented useEffect for API calls', {
        tags: ['react', 'hooks', 'useEffect', 'api']
      });
      
      // Day 4: Semantic memory - generalized knowledge
      await semanticMemory.store(userId, agentId, 'React hooks manage state and side effects in functional components', {
        keywords: ['react', 'hooks', 'state', 'functional-components'],
        confidence: 0.85
      });
      
      // Day 5: Procedural memory - learned patterns
      await proceduralMemory.learn(userId, agentId, 'need state in React component', 'use useState hook');
      await proceduralMemory.learn(userId, agentId, 'need side effect in React', 'use useEffect hook');
      
      // Verify learning progression
      const episodicExperiences = await episodicMemory.recall(userId, agentId, 'React', { limit: 10 });
      expect(episodicExperiences).toHaveLength(2); // Both React experiences should be found
      
      const semanticKnowledge = await semanticMemory.search(userId, agentId, 'React hooks');
      expect(semanticKnowledge).toHaveLength(1);
      
      const patterns = await proceduralMemory.getRecommendedActions(userId, agentId, 'need state');
  ...
  describe('Error Handling and Resilience', () => {
    test('one memory type failure does not affect others', async () => {
      ...
      const workingId = await workingMemory.store(userId, agentId, 'Working memory success');
      const episodicId = await episodicMemory.store(userId, agentId, 'Episodic memory success');
      const semanticId = await semanticMemory.store(userId, agentId, 'Semantic memory success');
      
      // Mock storage error for procedural memory only
      const originalStore = storage.memory.store;
      storage.memory.store = jest.fn().mockImplementation((userId, agentId, data) => {
      ...
      await expect(proceduralMemory.store(userId, agentId, 'Procedural failure test'))
  ...
  describe('Performance and Scalability', () => {
    test('memory system handles moderate load across all types', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        promises.push(workingMemory.store(userId, agentId, `Working memory ${i}`));
        promises.push(episodicMemory.store(userId, agentId, `Episodic memory ${i}`));
        promises.push(semanticMemory.store(userId, agentId, `Semantic memory ${i}`));
        promises.push(proceduralMemory.store(userId, agentId, `Procedural memory ${i}`));
      }
