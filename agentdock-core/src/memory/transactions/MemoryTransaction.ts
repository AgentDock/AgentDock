...
export interface Transaction {
  ...
  addOperation(forward: () => Promise<void>, rollback: () => Promise<void>): void;
}
...
export class MemoryTransaction implements Transaction {
  ...
  async commit(): Promise<void> {
    ...
    try {
      ...
      // Rollback on failure
      logger.error(LogCategory.STORAGE, 'MemoryTransaction', 'Transaction commit failed, rolling back', {
  ...
  async rollback(): Promise<void> {
    ...
    for (let i = this.executed - 1; i >= 0; i--) {
      try {
        ...
        logger.error(LogCategory.STORAGE, 'MemoryTransaction', 'Rollback operation failed', { 
          operation: i,
    ...
    if (failedRollbacks.length > 0) {
      logger.warn(LogCategory.STORAGE, 'MemoryTransaction', 'Some rollback operations failed', {
