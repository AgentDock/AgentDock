 ...
 */
export function formatAuthorAsMarkdown(
  ...
  const metricsSection = metrics.length > 0 ? `## Metrics\n\n${metrics.join('\n\n')}` : '';
  ...
  return [name, metricsSection, affiliations, linksSection, papersSection, attribution]
    .filter(Boolean)
    .join('\n\n');
}
