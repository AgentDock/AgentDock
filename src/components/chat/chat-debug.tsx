  ...
  ({
    ...
    useEffect(() => {
      if (typeof window !== 'undefined') {
        ...
        if (
          process.env.NEXT_PUBLIC_DEFAULT_HISTORY_POLICY ||
    ...
    const agentTemplate = agentId
      ? (templates[agentId as keyof typeof templates] as unknown as AgentTemplate)
      : undefined;
      ...
      <>
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          <SheetContent
            ...
            className={cn(
              'w-full',
              isMobile ? 'h-[70vh] rounded-t-lg border-t' : 'sm:max-w-md',
              'flex flex-col',
            )}
          ...
          >
            <SheetHeader className="text-left border-b pb-4">
              ...
              <SheetDescription className="text-xs">
                Details about the current chat session
              </SheetDescription>
            </SheetHeader>
            ...
            <div className="flex-1 overflow-y-auto space-y-6 p-4">
              ...
              <DebugSection title="Session">
                <DebugItem
                  label="Session ID"
                  value={fetchedState?.sessionId || sessionId || 'N/A'}
                />
              ...
              <DebugSection title="Cumulative Session Usage">
                ...
                {fetchError && <div className="text-sm text-destructive">Error: {fetchError}</div>}
                {!isLoadingUsage && !fetchError && fetchedState?.cumulativeTokenUsage && (
                  <div className="font-mono text-sm bg-muted p-2 rounded space-y-1">
                    ...
                    <div>
                      Completion Tokens: {fetchedState.cumulativeTokenUsage.completionTokens}
                    </div>
                    <div className="font-semibold">
                      Total Tokens: {fetchedState.cumulativeTokenUsage.totalTokens}
                    </div>
                  </div>
                )}
                {!isLoadingUsage && !fetchError && !fetchedState?.cumulativeTokenUsage && (
              ...
              {(activeStep || fetchedState?.activeStep) && (
                <DebugSection title="Current Orchestration Step">
                  <DebugItem
                    label="Active Step"
                    value={activeStep || fetchedState?.activeStep || 'N/A'}
                  />
